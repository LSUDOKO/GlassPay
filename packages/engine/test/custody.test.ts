...
beforeAll(() => {
  process.env.GLASSPAY_MASTER_KEY = "a".repeat(64); // test-only master key
});
...
describe("envelope encryption", () => {
  ...
  test("wrong master key -> throws", async () => {
    const blob = await encryptSecret("secret");
    process.env.GLASSPAY_MASTER_KEY = "b".repeat(64);
    await expect(decryptSecret(blob)).rejects.toThrow();
    process.env.GLASSPAY_MASTER_KEY = "a".repeat(64);
  });
});
