...
import { KeyedMutex, Relayer, Store, type DelegationSigner, type FinalizeOpsDeps, type SpendDeps } from "@glasspay/engine";
...
export function realDeps(): AppDeps {
  const store = new Store(); // GLASSPAY_DB_PATH or :memory:
  const relayer = new Relayer();
  const pk = process.env.GLASSPAY_DEV_USER_PK as Hex | undefined;
  const privyAppId = process.env.GLASSPAY_PRIVY_APP_ID;
  const deps: AppDeps = {
    store,
    relayer,
    userSigner: pk ? privateKeyToAccount(pk) : null,
    adminToken: process.env.GLASSPAY_ADMIN_TOKEN ?? null,
  ...
  const settleAddr = process.env.GLASSPAY_SETTLEMENT_ADDRESS;
  let settlementOk = process.env.GLASSPAY_FIAT_SETTLEMENT === "1";
  if (settlementOk && settleAddr && !isAddress(settleAddr)) {
    console.error("[deps] GLASSPAY_SETTLEMENT_ADDRESS is not a valid address; on-chain settlement DISABLED (fiat lane falls back to ledger-only)");
    settlementOk = false;
  }
