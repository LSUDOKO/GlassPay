...
import { hashCardSecret, type CardRow } from "@glasspay/engine";
...
export function mcpRoutes(deps: AppDeps, oauth: OAuthStore): Hono {
  ...
  const badSecret = new RateLimiter(envInt("GLASSPAY_MCP_BAD_SECRET_LIMIT", 30), 60_000);
  // per-card request ceiling: 240 calls / minute (well above any real agent loop)
  const perCard = new RateLimiter(envInt("GLASSPAY_MCP_RATE_LIMIT", 240), 60_000);

  // host allowlist (DNS-rebinding): enforced only when an explicit public base is set.
  // GLASSPAY_ALLOWED_HOSTS (comma-separated) widens it, e.g. the Railway fallback domain.
  // Evaluated per request (cheap) so env changes don't require a route rebuild.
  const allowedHosts = (): Set<string> | null => {
    try {
      const fromBase = process.env.GLASSPAY_PUBLIC_MCP_BASE ? [new URL(process.env.GLASSPAY_PUBLIC_MCP_BASE).host] : [];
      const extra = (process.env.GLASSPAY_ALLOWED_HOSTS ?? "").split(",").map((h) => h.trim()).filter(Boolean);
