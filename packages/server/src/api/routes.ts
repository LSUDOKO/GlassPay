...
//   - ADMIN (ops): Bearer GLASSPAY_ADMIN_TOKEN · full access, server-side curl/scripts.
...
//     "glasspay-onboard:v1:<did>" (personal_sign), so a Privy login can only ever claim
...
export function apiRoutes(deps: AppDeps, oauth: OAuthStore): Hono<{ Variables: { auth: AuthCtx } }> {
  ...
  app.use("*", async (c: Context<{ Variables: { auth: AuthCtx } }>, next: Next) => {
    if (!deps.adminToken && !deps.verifyPrivyToken) {
      return c.json({ error: "api disabled (no GLASSPAY_ADMIN_TOKEN and no GLASSPAY_PRIVY_APP_ID)" }, 503);
    }
  ...
  // over "glasspay-onboard:v1:<did>"): possession of the key, bound to THIS login.
  app.post("/onboard", (c) =>
    handle(c, async () => {
  ...
  app.post("/cards", (c) =>
    handle(c, async () => {
      ...
      if (!deps.userSigner) throw new EngineError("api", "no dev signer configured (GLASSPAY_DEV_USER_PK)");
