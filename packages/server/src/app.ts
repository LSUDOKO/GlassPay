...
export function createApp(deps: AppDeps): Hono {
  ...
  app.use(
    ...
    cors({
      origin: (origin) => {
        const allowed = (process.env.GLASSPAY_CORS_ORIGINS ?? "http://localhost:4071").split(",");
  ...
  app.route(
    "/",
    sellerRoutes(deps, () => process.env.GLASSPAY_FACILITATOR_BASE ?? `http://localhost:${process.env.PORT ?? 4070}/facilitator`),
  );
