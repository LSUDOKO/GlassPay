...
import { KeyedMutex, Store, type Relayer } from "@glasspay/engine";
...
describe("/cards/compile route", () => {
  ...
  const savedMasterKey = process.env.GLASSPAY_MASTER_KEY;
  beforeAll(() => {
    process.env.GLASSPAY_MASTER_KEY = "e".repeat(64);
  });
  afterAll(() => {
    if (savedMasterKey === undefined) delete process.env.GLASSPAY_MASTER_KEY;
    else process.env.GLASSPAY_MASTER_KEY = savedMasterKey;
  });
