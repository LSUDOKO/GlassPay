...
beforeAll(() => {
  process.env.GLASSPAY_MASTER_KEY = "e".repeat(64);
  process.env.GLASSPAY_PAID_FETCH_ALLOW_LOCAL = "1";
  ...
  process.env.GLASSPAY_FACILITATOR_BASE = `${base}/facilitator`;
  store.upsertUser({ id: "u-x402", address: user.address });
});

afterAll(() => {
  server.stop(true);
  delete process.env.GLASSPAY_PAID_FETCH_ALLOW_LOCAL;
});
...
describe("the full paid_fetch loop", () => {
  ...
  test("max_price refusal; non-402 passthrough; SSRF guard", async () => {
    ...
    process.env.GLASSPAY_PAID_FETCH_ALLOW_LOCAL = "";
