...
import { FEE_COLLECTOR, KeyedMutex, Store, issueRootCard, issueSubCard, freezeCard, unfreezeCard, type Relayer } from "@glasspay/engine";
...
beforeAll(async () => {
  process.env.GLASSPAY_MASTER_KEY = "f".repeat(64);
  process.env.GLASSPAY_STRIPE_WEBHOOK_SECRET = WHSEC;
...
afterAll(() => {
  server.stop(true);
  delete process.env.GLASSPAY_STRIPE_WEBHOOK_SECRET;
});
...
describe("the 2s decision", () => {
  ...
  test("frozen card declines; unknown mapping declines; non-usd declines", async () => {
    ...
    const { unfreezeCard } = await import("@glasspay/engine");
...
describe("fiat charges + settlement mode", () => {
  ...
  test("settlement mode: approved row lands pending with a recipient and fires the settler", async () => {
    ...
    process.env.GLASSPAY_FIAT_SETTLEMENT = "1";
    delete process.env.GLASSPAY_SETTLEMENT_ADDRESS; // recipient falls back to the fee collector
    deps.fiatSettler = {
      settle: async (cid) => {
        settles.push(cid);
      },
    ...
    try {
      ...
      delete process.env.GLASSPAY_FIAT_SETTLEMENT;
